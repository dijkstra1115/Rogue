...
##
## 原理：
##   - 每 tick 送出輸入的同時，立刻用與伺服器相同的 PlayerSim.step 在本地模擬
##     （這就是「本地零輸入延遲」的來源；衝刺也在這裡被預測）
##   - 保留未被伺服器確認的輸入，以及逐 tick 的「移動狀態快照」
##   - 伺服器狀態帶著 ack（該玩家最後被消化的輸入 tick）與移動快照回來：
##       位置一致 → 清掉舊資料；不一致 → 用伺服器快照回滾，重放 ack 之後的輸入
...
## 輸入 tick -> 當時的移動狀態快照（PlayerSim.snapshot_movement）
...
## 收到伺服器狀態時呼叫：比對並在必要時回滾＋重放（就地修改 state）。
## server_snap = 伺服器下發的移動狀態快照；ack_tick = 最後被消化的輸入 tick。
func reconcile(
	state: Dictionary,
	...
	if predicted != null \
			and (predicted.pos as Vector2).distance_to(server_snap.pos) < RECONCILE_THRESHOLD:
		return   # 猜對了，什麼都不用做

	# 猜錯了（或沒有對應歷史，例如剛出生）：用伺服器快照回滾，重放未確認的輸入
	if predicted != null:
		correction_count += 1
